...
const ReviewSection = ({ providerId }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await axios.get(`https://backend-final-lddn.onrender.com/api/providers/${providerId}`);
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.post(
        `https://backend-final-lddn.onrender.com/api/providers/${providerId}/reviews`,
        { rating, comment },
